...
namespace Parkner.Web.Pages.Clientes
{
    ...
    public partial class Listar
    {
        ...
        private async Task RecibirAsync()
        {
            try
            {
                ListaPaginada<Cliente> clientesPaginados = await this.ServicioClientes.ObtenerTodosAsync(this.PaginacionPeticion);
                this.Clientes = clientesPaginados.Lista;
                this.PaginacionMetaData = clientesPaginados.MetaData;
            }
            catch (Exception excepcion)
            {
                this.Registro.LogError($"Error al obtener clientes paginados /clientes: {excepcion.Message}");
                this.Mensajes.MostrarError(excepcion.Message);
            }
        }

        private void Crear() => this.Navegacion.NavigateTo("/clientes/crear");
    }
}
